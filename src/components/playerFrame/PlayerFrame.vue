...
watch(isPlayerTurn, function turnCheck() {
  ...
  const wontBet = store.getters.gameStage === GameStages.PlaceBets && props.player.isDealer;
...
watch(() => store.getters.dealer.score, function endRound(dealerScore: number) {
...
watch (() => store.getters.gameRound,function cleanUp() {
  roundResult.value = "";
...
<template>
  <section class="player-frame flex flex-column" :class="playerClass" ref="frameParent" >
    ...
    <header class="player-frame-title flex frame shadow-light" v-if="!props.player.isDealer" :class="{ 'is-active' : isPlayerTurn  }" >
      <h4 class="player-name" :class="{ 'alert-text': isPlayerTurn, 'error-text': !props.player.inGame }" >{{props.player.name}}</h4>
